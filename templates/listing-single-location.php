...
if( $lat && $long && ! $hide ) { ?>
	...
	<div class="wpsight-listing-section wpsight-listing-section-location">
		...
		?>
        <?php  if ( wpsight_get_option( 'listings_map_provider' ) == 'google') { ?>
            <script>
              function initialize() {
    ...
    <?php  if ( wpsight_get_option( 'listings_map_provider' ) == 'leaflet') { ?>
        <script>
            (function ($) {
                var map = L.map(document.getElementById('map-canvas'), {
                    center: [
                        <?php echo $lat; ?>,
                        <?php echo $long; ?>
                    ],
                    zoom: <?php echo  get_post_meta( get_the_ID(), '_map_zoom', true ) ? get_post_meta( get_the_ID(), '_map_zoom', true ) : 14 ?>,
                    scrollWheelZoom: false,
                });


                var mapType ="<?php echo get_post_meta( get_the_ID(), '_map_type', true ); ?>";

                var customLayer = L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
                    attribution: null
                });
                if( mapType === 'TERRAIN' ) {
                ...
                var latLng = new L.latLng( <?php echo $lat; ?>,  <?php echo $long; ?>);
                new L.Marker(latLng).setLatLng(latLng).addTo(map);
            })(jQuery);
        </script>
    <?php } ?>
<?php } // endif $location ?>