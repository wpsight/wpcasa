...
//		add_action( 'install_themes_wpcasa_themes',		array( $this, 'themes_page' ), 10, 1 );
    ...
    public function admin_enqueue_scripts() {
        ...
        if ( in_array( $screen->id, array( 'edit-' . $post_type, $post_type, 'toplevel_page_wpsight-settings', 'wpcasa_page_wpsight-addons', 'wpcasa_page_wpsight-themes', 'wpcasa_page_wpsight-licenses', 'wpcasa_page_wpsight-recommends' ) ) || $screen->id == 'plugin-install' && isset( $_GET['tab'] ) && $_GET['tab'] == 'wpcasa_addons' ) {
            ...
            wp_enqueue_style( 'wpsight-admin-swiper-styles',		WPSIGHT_PLUGIN_URL . '/assets/css/swiper.min' . $suffix . '.css', array( 'cmb2-styles' ) );
            wp_enqueue_script( 'wpsight-admin-swiper-script',				WPSIGHT_PLUGIN_URL . '/assets/js/swiper.min' . $suffix . '.js', array( 'jquery' ), WPSIGHT_VERSION, true );
